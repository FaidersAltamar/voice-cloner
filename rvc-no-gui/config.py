...
class TrainingConfig:
    ...
    # GPU settings
    use_gpu: bool = field(default_factory=lambda: PLATFORM.has_cuda)
    gpu_ids: str = "0"
    cache_data_in_gpu: bool = False

    def get_device(self) -> str:
        """Device for training/features: cuda:0 or cpu."""
        if not self.use_gpu:
            return "cpu"
        return f"cuda:{self.gpu_ids.split('-')[0]}" if self.gpu_ids else "cpu"
