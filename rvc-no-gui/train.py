...
class RVCTrainer:
    ...
    def _generate_filelist(self, model_name: str) -> bool:
        ...
        # Add mute entries (only if mute directory exists)
        fea_dim = 256 if version == "v1" else 768
        mute_dir = self.paths.logs_dir / "mute"
        mute_gt = mute_dir / "0_gt_wavs" / f"mute{sample_rate}.wav"
        if mute_gt.exists():
            for _ in range(2):
                mute_gt_str = str(mute_gt).replace("\\", "\\\\")
                ...
                if use_f0:
                    ...
                    opt.append(f"{mute_gt_str}|{mute_feature}|{mute_f0}|{mute_f0nsf}|{speaker_id}")
                else:
                    opt.append(f"{mute_gt_str}|{mute_feature}|{speaker_id}")
    ...
    def _run_training(
        ...
        # Entorno: forzar CPU si use_gpu=False
        env = os.environ.copy()
        if not self.training_config.use_gpu:
            env["CUDA_VISIBLE_DEVICES"] = ""
            gpu_arg = ""  # RVC pondrá CUDA_VISIBLE_DEVICES="" → sin GPU
        else:
            gpu_arg = self.training_config.gpu_ids
