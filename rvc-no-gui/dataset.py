...
class DatasetPreparer:
    ...
    def _handle_resume_training(self, model_name: str):
        ...
        # Estrategia: renombrar todo el directorio (evita PermissionError en Windows)
        backup_name = f"{model_name}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        backup_path = logs_path.parent / backup_name
        if backup_path.exists():
            shutil.rmtree(backup_path)

        try:
            # Mover directorio completo a backup (rename evita borrar archivos bloqueados)
            logs_path.rename(backup_path)
        except (PermissionError, OSError) as e:
            logger.warning(f"Rename failed ({e}), trying file-by-file cleanup...")
            self._handle_resume_training_fallback(logs_path)
            return

        # Crear directorio limpio
        logs_path.mkdir(parents=True)

        # Copiar solo D_/G_ de vuelta
        for item in backup_path.iterdir():
            if item.is_file() and (item.name.startswith('D_') or item.name.startswith('G_')) and item.suffix == '.pth':
                shutil.copy(item, logs_path / item.name)
                logger.info(f"Restored checkpoint: {item.name}")

        # Intentar borrar backup (puede fallar si hay locks; se deja para limpieza manual)
        try:
            shutil.rmtree(backup_path)
        except PermissionError:
            logger.info(f"Backup kept at {backup_path} (delete manually if needed)")

    def _handle_resume_training_fallback(self, logs_path: Path):
        """Fallback: borrar archivos uno a uno cuando rename falla."""
        ...
        for item in list(logs_path.iterdir()):
            if item.is_file() and (item.name.startswith('D_') or item.name.startswith('G_')) and item.suffix == '.pth':
                ...
                checkpoint_names.append(item.name)

        trash_dir = logs_path / "_trash"
            ...
            for attempt in range(10):
                try:
                    ...
                    if attempt < 9:
                        ...
                        try:
                            shutil.move(str(item), trash_dir / item.name)
                        except Exception:
                            raise PermissionError(
                                f"No se puede borrar ni mover {item.name}. "
                                "Cierra otros programas que lo usen o borra manualmente la carpeta "
                                f"logs/{logs_path.name}"
                            )

        for name in checkpoint_names:
            shutil.move(str(temp_dir / name), logs_path / name)
    ...
    def _extract_features(self, model_name: str) -> bool:
        ...
        device = self.training_config.get_device()
        cmd = [
            sys.executable,
            str(script),
            device,
            ...
            "True" if "cuda" in device else "False",  # is_half solo en GPU
        ]

        env = None
        if not self.training_config.use_gpu:
            env = os.environ.copy()
