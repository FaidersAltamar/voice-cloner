...
class RVCPipeline:
    ...
    def run_full_training(
        ...
        Args:
            ...
            use_gpu: True=GPU, False=CPU, None=auto (GPU si está disponible)
        ...
        if use_gpu is not None:
            self.config.training.use_gpu = use_gpu
        # F0 y features según device
        if self.config.training.use_gpu and PLATFORM.has_cuda:
            self.config.f0.method = "rmvpe_gpu"
        else:
            self.config.f0.method = "rmvpe"
...
Examples:
  ...
  python pipeline.py list
    ...
    train_parser.add_argument("--device", "-d", choices=["auto", "gpu", "cpu"], default="auto",
                              help="Device: auto (GPU si disponible), gpu, cpu (default: auto)")
...
def main():
    ...
    if args.command == "setup":
        ...
        use_gpu = {"auto": None, "gpu": True, "cpu": False}[args.device]
